 ...
 * 期ごとに表示・編集する。
 * 列: 名前 | 入社日 | 上期(所属/兼務先/兼務率/夏ボ/7〜12月/上期計)
...
export function LaborSheetTab({ term }: { term: TermCode }) {
  ...
  const deptOptions = useMemo(() => {
    ...
    for (const m of deptMap) set.add(m.dept);
  ...
  const columns: GridColumn[] = useMemo(() => {
    const half = (h: Half, label: string, amountCols: { key: Slot; title: string }[]): GridColumn[] => [
      { key: `${h}:dept`, title: `${h === "H1" ? "上期" : "下期"}所属`, width: 110, type: "text", group: label, options: deptOptions },
      { key: `${h}:kenmu`, title: "兼務先", width: 100, type: "text", group: label, options: deptOptions },
  ...
  const rows: GridRow[] = useMemo(() => {
    ...
    return out;
  }, [people, assignments, amounts, term, showAll]);
  ...
  const onEdits = (edits: GridEdit[], label: string) => {
    ...
    for (const e of edits) {
      ...
      if (e.colKey === "hired") {
        void useLaborCostStore.getState().updatePerson(e.rowId, {
  ...
  return (
    <div className="labor-sheet">
      ...
      <p className="labor-hint">
        コピー/ペースト（⌘C/⌘V・スプレッドシート互換）・⌘Z 取り消し・⌘D フィルダウン・
        Delete クリア・ドラッグ/Shift+クリックで範囲選択。金額は万円。
