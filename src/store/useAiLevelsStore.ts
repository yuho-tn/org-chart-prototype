...
function isMissingTable(message: string | undefined | null): boolean {
  return (
    !!message &&
    /does not exist|could not find the (table|function)|schema cache/i.test(message)
  );
}
...
export const useAiLevelsStore = create<AiLevelsState>((set, get) => ({
  ...
  refresh: async (opts) => {
    ...
    try {
      ...
      set({
        ...
        missing: false,
        error: null,
        ...withDerived((data ?? []) as AiLevelGrantRow[]),
      });
    } catch (e) {
  ...
  bulkImport: async (entries, opts) => {
    const summary: BulkSummary = { total: entries.length, inserted: 0, errors: [] };
    if (!supabase) {
    ...
    for (let i = 0; i < payload.length; i += CHUNK) {
      ...
      const { error } = await supabase.from("ai_level_grants").insert(slice);
      if (error) {
